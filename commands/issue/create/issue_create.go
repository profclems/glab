...
func NewCmdCreate(f *cmdutils.Factory) *cobra.Command {
	...
	carapace.Gen(issueCreateCmd).FlagCompletion(carapace.ActionMap{
		...
		"linked-issues": carapace.ActionMultiParts(",", func(c carapace.Context) carapace.Action {
			return action.ActionIssues(f, &gitlab.ListProjectIssuesOptions{State: gitlab.String("opened")}).Invoke(c).Filter(c.Parts).ToA()
		}),
		"linked-mr": action.ActionMergeRequests(f, &gitlab.ListProjectMergeRequestsOptions{State: gitlab.String("opened")}),
	})
