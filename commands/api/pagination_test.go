...
func Test_findNextPage(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			resp: &http.Response{
				Header: http.Header{
					"Link": []string{`<https://gitlab.com/api/v4/issues?page=3>; rel="last"`},
				},
			},
			want:  "",
			want1: false,
		},
		{
			...
			resp: &http.Response{
				Header: http.Header{
					"Link": []string{`<https://gitlab.com/api/v4/issues?page=2>; rel="next", <https://gitlab.com/api/v4/issues?page=3>; rel="last"`},
				},
			},
			want:  "https://gitlab.com/api/v4/issues?page=2",
			want1: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := findNextPage(&HTTPResponse{Response: &gitlab.Response{Response: tt.resp}})
			if got != tt.want {
